...
5. **Update affected files.** Refresh the client `README.md` (status, stakes, $, next action), `pipeline/_board.md`, and `operating-system/dashboard.html`. Capture new stakeholders, decisions, dates.
6. **Move files to match reality.** If the deal changed stage, run `consulting-deal-stage-mover`.
7. **Mine for content + proof.** Recurring answers → `knowledge-base/faqs/`; wins → trigger `consulting-case-study-builder` / `consulting-testimonial-capture`.
